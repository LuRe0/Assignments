...
namespace CS170 {
  ...
  {
      unsigned int j = size_ - 1;
      for (unsigned int i = 0; i < size_/2; i++)
      {
          swap_ints(array_[i], array_[j]);
          if (i == j)
          {
              break;
          }
  ...
  {
      swap_pointers(array_, other.array_);
      swap_unsigned(capacity_, other.capacity_);
      swap_unsigned(size_, other.size_);
      swap_unsigned(allocs_, other.allocs_);
  }
